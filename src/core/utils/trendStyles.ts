import type { PercentileGroup } from '../../../shared/percentiles';

export const DOT_STYLES: Record<PercentileGroup | 'default', { colorHex: string; tailwindColor: string; size: number }> = {
...
export const getDotStyle = (percentileGroup: string | undefined) => {
    const key = (percentileGroup && percentileGroup in DOT_STYLES)
        ? (percentileGroup as PercentileGroup | 'default')
        : 'default';
