...
export const AGENTIC_BEHAVIOR_RULES = [
    ...
    '- **Visual context.** `browseTrendVideos` and `getMultipleVideoDetails` return `thumbnailDescription` — an AI-generated visual summary (~200 words) for videos with embeddings. Use it to understand the visual ecosystem (style patterns, aesthetic categories, visual drift over time) without extra tool calls. For deep visual analysis (composition details, cloning verification, specific recommendations) — call `viewThumbnails`.',
    ...
    '',
    '### Deep Analysis → Save KI First',
    'When performing deep analysis (2+ analytical tools like `analyzeTrafficSources` + `analyzeSuggestedTraffic`):',
    '1. Analyze data in your thinking phase.',
    '2. Call `saveKnowledge` with the full comprehensive analysis.',
    '3. In chat, write only a brief summary: 3-5 key findings with numbers, main conclusion, and a `[KI Title](ki://kiId)` link.',
    '',
    'Never write the full analysis in chat and then ask "Save as KI?" — save first, summarize in chat.',
    'For quick lookups (single tool, simple question) — respond directly in chat without saving.',
    '',
    '### Visual Analysis in Knowledge Items',
    'For every video-scoped KI, include a **Thumbnail & Visual Identity** section immediately after the video metadata header:',
    '- **Primary video:** Call `viewThumbnails` to see the actual thumbnail image. Describe visual style, composition, color palette, text overlays, mood — then connect to performance patterns (algorithmic placement, audience drift, CTR).',
    '- **Referenced videos:** Include at least their `thumbnailDescription` (from `getMultipleVideoDetails`/`browseTrendVideos`). When visual comparison is central to your argument, call `viewThumbnails` for those too.',
    '- **Connect visuals to performance.** Don\'t just describe — hypothesize why the visual choices matter.',
    'Plan your tool calls accordingly: fetch visual data BEFORE calling `saveKnowledge`.',
    '',
    '### Response Quality',
...
export const VIDEO_CONTEXT_PREAMBLE = [
    ...
    '**View growth deltas** (24h/7d/30d) are **rolling windows calculated backward from today** — NOT from the video\'s publish date. viewDelta30d = views gained in the last 30 calendar days. For a video published 6 months ago, viewDelta30d shows its CURRENT monthly momentum, not its launch performance. Use these to assess momentum — a video with 1M total views but +100K in 7d is trending hard, while +200 in 7d means it has plateaued.',
