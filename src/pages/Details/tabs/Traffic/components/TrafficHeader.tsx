...
export const TrafficHeader: React.FC<TrafficHeaderProps> = ({
    ...
    return (
        <>
            <div className={`sticky top-0 z-dropdown px-6 py-4 transition-shadow duration-200 bg-video-edit-bg ${isScrolled ? 'shadow-[0_2px_8px_rgba(0,0,0,0.3)]' : ''}`}>
                <div className="flex items-center justify-between gap-4 max-w-[1200px]">
                    ...
                    {/* Actions container - always shows upload, conditionally shows table controls */}
                    <div className="flex gap-2">
                        {/* Table controls — only when data exists */}
                        {shouldShowActions && (
                            <>
                                {/* Smart Assistant Toggle */}
                                {onToggleAssistant && (
