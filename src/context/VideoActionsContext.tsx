...
export const useVideoActions = () => {
    ...
    if (!context) {
        throw new Error('useVideoActions must be used within a VideoActionsProvider');
    }
    return context;
};

export const VideoActionsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    const { user } = useAuth();
    const { currentChannel } = useChannel();
    const { videos } = useVideos(); // Consuming data from VideosContext
    const {
        generalSettings,
        videoOrder,
        updateVideoOrder,
    ...
    }, [syncSettings.autoSync, syncSettings.frequencyHours, videos.length, user, currentChannel, generalSettings.apiKey, isSyncing]);
    ...
    return (
        <VideoActionsContext.Provider value={{
            addVideo,
            addCustomVideo,
            updateVideo,
            removeVideo,
