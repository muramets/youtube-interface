...
export const SegmentedControl = <T extends string | number>({
    ...
    return (
        <div className={`relative flex bg-[var(--hover-bg)] dark:bg-[#1a1a1a] rounded-lg p-0.5 ${disabled ? 'opacity-50 cursor-not-allowed' : ''} ${className}`}>
            {/* Sliding Indicator */}
            <div
                className={`absolute top-0.5 bottom-0.5 bg-bg-primary dark:bg-gradient-to-r dark:from-[#2d2d2d] dark:to-[#333333] shadow-sm rounded-md transition-all duration-200 ease-out ${selectedIndex === -1 ? 'opacity-0' : 'opacity-100'
                    }`}
                style={{
