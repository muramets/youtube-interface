...
function makeMockProvider(label: string): AiProvider {
    return {
        ...
        generateText: vi.fn<(opts: GenerateTextOpts) => Promise<GenerateTextResult>>().mockResolvedValue({
...
/** Create a mock AiProvider WITHOUT generateText (simulates unsupported provider). */
function makeMockProviderWithoutGenerateText(label: string): AiProvider {
    return {
        streamChat: vi.fn<(opts: ProviderStreamOpts) => Promise<StreamResult>>().mockResolvedValue({
            text: `response from ${label}`,
            tokenUsage: { promptTokens: 10, completionTokens: 5, totalTokens: 15 },
...
describe("createProviderRouter — generateText routing", () => {
    it("routes generateText to the correct provider", async () => {
        const router = createProviderRouter({
            registry: {
                gemini: { factory: geminiFactory.factory, config: { apiKey: "gk" } },
                anthropic: { factory: anthropicFactory.factory, config: { apiKey: "ak" } },
        ...
        const result = await router.generateText({ model: "gemini-2.5-pro", text: "test" });

        expect(geminiFactory.provider.generateText).toHaveBeenCalledWith({ model: "gemini-2.5-pro", text: "test" });
        expect(result.text).toBe("generated by gemini");
    });

    it("throws for unknown model in generateText", async () => {
        const router = createProviderRouter({
            registry: {
                gemini: { factory: geminiFactory.factory, config: {} },
            },
        ...
        await expect(
            router.generateText({ model: "gpt-4o", text: "test" }),
        ).rejects.toThrow(/Unknown model "gpt-4o"/);
    });

    it("throws when provider does not support generateText", async () => {
        const noGenFactory = makeMockProviderWithoutGenerateText("limited");
        const limitedFactory = vi.fn<ProviderFactory>().mockReturnValue(noGenFactory);

        const router = createProviderRouter({
            registry: {
                limited: { factory: limitedFactory, config: {} },
            },
            modelToProvider: { test: "limited" },
        });

        await expect(
            router.generateText({ model: "test-model", text: "test" }),
        ).rejects.toThrow(/does not support generateText/);
    });
});
