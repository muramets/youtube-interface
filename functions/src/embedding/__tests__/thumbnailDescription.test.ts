...
const { mockGenerateContent, mockLoggerWarn } = vi.hoisted(() => ({
...
describe("generateThumbnailDescription", () => {
    ...
    it("returns description when given a valid thumbnail", async () => {
        mockGenerateContent.mockResolvedValueOnce({
            text: "A vibrant thumbnail showing a person pointing at a chart with bold red text overlay.",
        });

        const result = await generateThumbnailDescription("testVideoId", thumbnail(), "test-api-key");
