...
| Phase | Goal | Status |
|-------|------|--------|
| 0 | `generateText()` in AiProvider: interface + Gemini + Claude + router | DONE |
| 1 | Protected flag: type + Firestore field + UI toggle | DONE |
| 2 | Consolidation CF: prompt, schema, validation, `onCall` endpoint | DONE |
| 3 | Consolidation UI: modal (selection → generate → preview/edit → save) | DONE |
| FINAL | Double review-fix cycle (R1: Architecture, R2: Production Readiness) | DONE |

## Current Test Count

- **Frontend: 686 tests (49 files)** — verified via `npx vitest run --project frontend` (2026-03-23)
- **Backend: 962 tests (68 files)** — verified via `npx vitest run --project functions` (2026-03-23)
- **Total: 1648 tests (117 files)** — all passing
