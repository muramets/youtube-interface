...
2. Читает `trendStore.channels` и `channelStore.currentChannel` (императивно, вне React)
3. Вычисляет delta views через `computeVideoDeltas()` — сколько просмотров прибавилось за 24h, 7d, 30d
4. Патчит items: `delta24h`, `delta7d`, `delta30d`
...
Дельты берутся из **trend snapshots** — периодических снимков статистики каналов. Если для видео нет snapshot данных — item возвращается без delta полей (graceful degradation).
...
**Delta computation:** `src/core/utils/computeVideoDeltas.ts`
