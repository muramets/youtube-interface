...
Видео передаются **компактно** (title + metrics) → system prompt ~10K токенов на 150 видео. Description и tags доступны LLM через `getMultipleVideoDetails` tool on-demand.
...
- ✅ **Competitive Intelligence Этапы 1–3**: Layer 4 (Competition) — 5 инструментов (`listTrendChannels`, `browseTrendVideos`, `getNicheSnapshot`, `findSimilarVideos`, `searchDatabase`) дают AI полный доступ к данным конкурентов: browsing, niche analytics, semantic search по теме (text embeddings), визуальному сходству обложек (Vertex AI) и free-text поиску по базе. Подробнее: [Competitive Intelligence](./tools/layer-4-competition/competitive-intelligence.md).
...
- [x] `getVideoComments(videoId)` — комментарии под любым публичным видео (YouTube API, 1-3 quota units)
...
**User flow:** Пользователь открывает чат → ассистент знает ВСЁ о его видео, трендах, конкурентах. Контекст передаётся легковесно — только ID, LLM сам находит данные в базе. Можно прикрепить 500 видео из Suggested Traffic и не платить за 500 описаний — LLM загрузит нужные по мере обсуждения.

- [ ] Полноценный агент с thinking, Tools, Vector Search
- [ ] Контекст = вся Firestore база, мосты передают только ID
- [ ] LLM всегда видит обложку обсуждаемого видео
...
- [AI Chat Tools](./tools/README.md) — Telescope pattern: 13 инструментов в 4 слоях (Discovery → Detail → Analysis → Competition)
...
- [Competitive Intelligence](./tools/layer-4-competition/competitive-intelligence.md) — 5 инструментов для анализа конкурентов: browsing, niche analytics, semantic + visual search, free-text search
