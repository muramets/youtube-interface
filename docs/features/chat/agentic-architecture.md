...
| `toolCall` | Gemini вызвал tool | ToolCallBadge (pending, shimmer) |
| `toolResult` | Результат tool | ToolCallBadge (resolved, зелёный ✓) |
...
├── ToolCallBadge.tsx     # Pill: pending → resolved → error
