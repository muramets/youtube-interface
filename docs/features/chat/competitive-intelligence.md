...
**Стадия: проектирование.** Данные конкурентов есть (Trends feature), но AI не имеет инструментов до них добраться. Единственный путь — `browseChannelVideos`, который идёт в YouTube API (тратит квоту, не знает о Trends).
...
**Чеклист:**
- [ ] Инструмент `listTrendChannels`
- [ ] Вынос percentile алгоритма в `shared/percentiles.ts` (SSOT для frontend + backend)
- [ ] Инструмент `browseTrendVideos` с фильтрами и percentile расчётом
- [ ] Инструмент `getNicheSnapshot`
- [ ] View deltas для trend videos (через существующий `trendSnapshotService`)
- [ ] Документация каждого инструмента (`docs/features/chat/tools/`)
- [ ] Тесты
