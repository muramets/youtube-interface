...
**Реализовано.** Tool results из предыдущих turns реконструируются в нативном формате провайдера (Claude: `tool_use`/`tool_result` blocks; Gemini: `functionCall`/`functionResponse` parts). Chained tool calls через turns работают. `contextBreakdown.toolResults` вычисляется из реальных данных после agentic loop. `estimateTokens()` учитывает tool data в бюджете summarization.
...
| Файл | Назначение |
|------|-----------|
| `functions/src/services/ai/types.ts` | `HistoryMessage` — добавлено поле `toolCalls` |
| `functions/src/chat/aiChat.ts` | Mapper: читает `toolCalls` из Firestore; contextBreakdown fix |
| `functions/src/services/claude/streamChat.ts` | `buildHistory()` — реконструкция tool blocks |
| `functions/src/services/gemini/streamChat.ts` | `buildHistory()` — реконструкция function parts |
