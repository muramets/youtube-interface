...
**Cache-оптимизация: frozen snapshot.** L4 memories инжектируются в system prompt как `memoriesSnapshot` — замороженный snapshot на момент открытия разговора. Когда AI вызывает `saveMemory` mid-conversation, Firestore обновляется, но system prompt текущего разговора **не меняется**. Новый memory вступает в силу в следующем разговоре. Это предотвращает каскадную инвалидацию prompt cache (prefix-based cache ломается при любом изменении в system prompt → re-cache всего downstream контента). Подробнее: [Prompt Caching](./prompt-caching.md).
...
- `src/core/stores/chat/slices/navigationSlice.ts` — `setActiveConversation()` and `startNewChat()` freeze `memoriesSnapshot` from live `memories` state
