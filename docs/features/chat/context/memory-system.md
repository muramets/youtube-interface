...
**Video References (legacy memories):** Старые memories (до миграции на conclude-as-last-turn) содержат `videoRefs[]` — snapshot-ы видео (`{ videoId, title, ownership, thumbnailUrl }`), собранные кодом при создании. В UI они отображаются как chips с обложками, в system prompt — как `[id: videoId]` аннотации.
...
- Backend injects `saveMemory` tool (conclude-only) alongside standard tools
- AI calls `saveKnowledge` ×N + `saveMemory` ×1 as tool calls, visible in chat with badges
- `saveMemory` handler (`functions/src/services/tools/handlers/knowledge/saveMemory.ts`): idempotency guard (60s window), orphan guard, saves to `conversationMemories` collection
...
**Store integration:**
- `src/core/stores/chat/slices/sendSlice.ts` — `resumeSendFlow()` passes `isConclude` through to backend; `sendMessage()` supports `SendOptions` with `backendText` (actual prompt) separate from display text
- `src/core/stores/chat/slices/settingsSlice.ts` — `memorizeConversation()` composes display text + `CONCLUDE_INSTRUCTION` and delegates to `sendMessage()`
