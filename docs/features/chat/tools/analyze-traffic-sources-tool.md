...
- [analyzeSuggestedTraffic](./analyze-suggested-traffic-tool.md) — drill-down после этого gateway
- [getMultipleVideoDetails](./get-multiple-video-details-tool.md) — `trafficSourceSnapshotCount` для pre-check
...
| Файл | Назначение |
|------|-----------|
| `functions/src/services/tools/handlers/analyzeTrafficSources.ts` | Handler: Firestore → Cloud Storage → parse → timeline → JSON |
