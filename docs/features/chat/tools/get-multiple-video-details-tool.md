...
```
resolveVideosByIds (videos/ + publishedVideoId reverse lookup + cached_external_videos/) → YouTube API (1 unit / 50 videos)
```
...
4. **YouTube API** — fallback, без отдельного quota gate (micro-cost: 1 unit на 50 видео)
...
| Файл | Назначение |
|------|-----------|
| `functions/src/services/tools/handlers/getMultipleVideoDetails.ts` | Handler: cascade via resolveVideosByIds, formatting, denormalized counts, view delta enrichment |
