...
4. LLM получает полные метаданные + `ownership` + traffic snapshot counts + view deltas + `thumbnailDescription` (visual context) + `youtubeVideoId` (для custom-видео)
...
```typescript
{
    videos: [{
        ...
        // AI-generated thumbnail visual summary (~200 words) from globalVideoEmbeddings
...
Для видео с embedding data (обработанных embedding pipeline) ответ может содержать:
- `thumbnailDescription` — AI-generated текстовое описание обложки (~200 слов: композиция, цвета, стиль, объекты, настроение)

Данные берутся из `globalVideoEmbeddings/{videoId}` через `fetchThumbnailDescriptions`. Доступны для конкурентных видео из tracked каналов. Для custom-видео lookup идёт по `youtubeVideoId` (publishedVideoId).

Это **surface-level** визуальный контекст: достаточно для понимания визуальной экосистемы, паттернов стиля, визуального дрифта. Для **глубокого** визуального анализа (верификация клонирования, конкретные рекомендации по дизайну) — LLM вызывает `viewThumbnails`.

Graceful degradation: если `globalVideoEmbeddings` недоступен или embedding отсутствует — поле просто не включается в ответ.
...
| Файл | Назначение |
|------|-----------|
| `functions/src/services/tools/handlers/detail/getMultipleVideoDetails.ts` | Handler: title resolution + cascade via resolveVideosByIds, formatting, denormalized counts, view delta + thumbnail description enrichment, youtubeVideoId |
| `functions/src/services/tools/utils/fetchThumbnailDescriptions.ts` | Batch fetch `thumbnailDescription` from `globalVideoEmbeddings` (shared utility) |
...
| `functions/src/services/tools/handlers/detail/__tests__/getMultipleVideoDetails.thumbnailDescriptions.test.ts` | 5 (regular video, custom published via youtubeVideoId, not available, failure graceful, custom- ID filtering) |
