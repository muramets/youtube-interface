...
### Шаг 5b — Единое чтение cached_external_videos + enrichment signal

Для всех topSources (не первых 30, как раньше — весь depth limit) читаются `cached_external_videos` **одним запросом**. Из этого чтения извлекаются:
1. **`enriched` флаг** на каждом topSource — `true` если есть полезные данные (tags непустые ИЛИ channelTitle непустой)
2. **`enrichmentCoverage: { enriched, total }`** в корне response
3. **`channelId` hints** для view delta enrichment
4. **`enrichedData`** Map (tags, channelTitle) для content analysis

Ранее handler делал два отдельных чтения одной коллекции — теперь одно.

### Шаг 5c — View Delta Enrichment
...
1. Использует `channelId` hints из шага 5b
...
Использует `enrichedData` Map из шага 5b (уже содержит все topSources). Дополнительно дочитывает ВСЕ видео из всех снапшотов (self-channel + trajectory) — batch по 500.
...
- `topSourceChannels` — какие каналы чаще появляются среди top sources
...
- topKeywords, topSharedTags, snapshotChannels — для каждого снапшота
...
```typescript
{
    ...
    enrichmentCoverage: { enriched: number, total: number },  // how many topSources have YouTube API metadata
    topSources: [{
        videoId, sourceTitle,
        views, impressions, ctr, avgViewDuration, watchTimeHours,
        enriched: boolean,             // has useful data in cached_external_videos (tags or channelTitle)
    ...
    contentAnalysis?: {
        ...
        aggregate: {
            ...
            topSourceChannels: [{ channelTitle, count }]    // renamed: channels among top sources only
        }
    },
    selfChannelStats?: {
    ...
    contentTrajectory?: [{
        ...
        snapshotChannels: [{ channelTitle, count }],    // renamed: channels across ALL sources in snapshot
...
| Какие каналы-конкуренты? | `contentAnalysis.topSourceChannels` (top sources) / `contentTrajectory[].snapshotChannels` (full pool) |
...
| Насколько полны данные? | `enrichmentCoverage` + `topSources[].enriched` — LLM знает для каких видео теги/каналы надёжны |
...
| `contentAnalysis` | perVideoOverlap[depth limit] + aggregate (tags, keywords, channels) |
...
| `contentAnalysis` (aggregate + perVideoOverlap, full depth) | ~2K | Частично (keywords) |
...
| `functions/src/services/tools/handlers/analysis/__tests__/analyzeSuggestedTraffic.viewDeltas.test.ts` | 3 (view delta enrichment + enriched flag + enrichmentCoverage) |
