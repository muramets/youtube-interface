...
|---|----------|---------------|-------------|-----------|
| 1 | **Happy path (standard)** | Базовый вызов: topSources, transitions, content analysis | "Какие видео YouTube рекомендует рядом с моим [X]?" | — |
...
| 4 | **No data (snapshotCount = 0)** | Проверяет ли модель suggestedTrafficSnapshotCount | "Какие видео рядом с [видео без CSV]?" | — |
| 5 | **Transitions interpretation** | Видит ли модель ротацию пула (newCount, droppedCount) | "Как менялось окружение моего видео?" | — |
...
| 8 | **View deltas usage** | Использует ли модель viewDelta24h/7d/30d для оценки source health | "Какие из рекомендованных видео сейчас растут?" | — |
| 9 | **mentionVideo calls** | Вызывает ли модель mentionVideo для видео из topSources | (покрывается любым trace) | — |
| 10 | **Token budget** | Помещается ли tool result в context window при разных depth | (покрывается traces #1 и #3) | — |
| 11 | **Long tail (tail field)** | Упоминает ли модель truncated sources | (покрывается trace #1) | — |

### Ключевые вопросы

1. **Token budget при standard depth** — 50 видео × 9 снэпшотов. Поместится? Сколько % context window?
2. **Depth auto-selection** — tool description не говорит модели как выбирать depth. Угадает ли Haiku, что "глубокий анализ" = `detailed`, а "быстрый обзор" = `quick`?
3. **Transitions vs timeline** — модель получает ДВА способа увидеть динамику: per-video timeline (индивидуальные траектории) и transitions (ротация пула). Сможет ли модель синтезировать оба?
4. **Self-channel interpretation** — `analysisGuidance` содержит текстовую подсказку (>60% = ecosystem boost). Прочитает ли модель её и использует?
5. **mentionVideo incentive** — tool description говорит "use mentionVideo to reference key competitor videos from topSources". Выполнит ли Haiku? (Из getNicheSnapshot traces: Haiku нестабилен с mentionVideo)
6. **viewDelta enrichment** — viewDelta полезен только для видео, чей канал tracked в Trends. Для остальных = null. Не запутается ли модель в null значениях?

### Проверено в бою

*(пока не проверено — секция будет заполнена после реальных traces)*

### Связь с analyzeTrafficSources battle test

Из [analyzeTrafficSources trace #2](./1-analyze-traffic-sources-tool.md): Browse features (63%) > Suggested videos (29%). Модель написала рекомендацию "углубитесь в Suggested traffic" — но НЕ вызвала `analyzeSuggestedTraffic` сама. Это тест #2 (Gateway chain): следует ли модель gateway-паттерну автоматически или только по явному запросу пользователя?
...
- **Нет share % в topSources** — как и в analyzeTrafficSources, модель должна считать долю каждого source от total impressions. Trace покажет, справляется ли
- **contentTrajectory дублирует topSources** — `topVideos` в trajectory и `topSources` содержат пересекающиеся данные. Возможный token waste
