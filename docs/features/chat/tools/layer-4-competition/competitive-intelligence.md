...
| **browseTrendVideos** — "что публиковали?" | "Что конкуренты выпустили на прошлой неделе?" "Топ-видео канала X?" "Что растёт быстрее всего?" | Список видео с фильтрами по каналу, дате, перформансу. Каждое видео — с тиром (Top 1%...Bottom 20%), динамикой роста (24ч/7д/30д) и `thumbnailDescription` (visual context) | Каналы, даты, тир, сортировка, лимит — всё опционально | Бесплатно | listTrendChannels → **browseTrendVideos** → viewThumbnails (для глубокого visual analysis) |
...
- **Создание + обновление:** исключительно через `scheduledEmbeddingSync`. Sync pipeline пишет в embedding queue (dirty detection), но НЕ пишет в `globalVideoEmbeddings` — изоляция сохраняется. EmbeddingSync читает queue (~5-20 entries/день вместо full scan ~5K reads), обрабатывает через `processOneVideo`, чистит queue. Fallback на full scan при первом запуске. Стоимость discovery: ~20 reads/день
...
```
scheduledSync (00:00 UTC) — ОБНОВЛЁН: dirty detection + embedding queue
  → SyncService.syncChannel()
    → YouTube API → save videos to Firestore
    → Pre-read existing docs (db.getAll) → compare 4 content fields (title, tags, description, thumbnail)
    → Changed/new videos → enqueue to system/embeddingQueue/videos/{videoId} (same batch = atomic)
    → save snapshots, update channel stats

scheduledEmbeddingSync (00:30 UTC) — ОБНОВЛЁН: queue-based (incremental)
  Thin launcher → Cloud Task chain:
  1. Read embedding queue (system/embeddingQueue/videos) → ~5-20 entries vs ~5000+ full scan
  2. If queue empty + embeddings exist → nothing to do, return
  3. If queue empty + NO embeddings → fallback to full scan (first run via discoverChannels)
  4. Записать `system/syncState` (channelPaths, videos[], atomic counters)
  5. Enqueue первый batch → embeddingSyncBatch(offset: 0)
...
embeddingSyncBatch(offset) — self-chaining batch processor:
  ...
  5. Queue cleanup: delete successfully processed videos from queue (generated + alreadyCurrent)
     Failed videos remain for retry on next run (idempotent — processOneVideo returns alreadyCurrent)
  6. If есть ещё videos → enqueue Cloud Task: embeddingSyncBatch(offset+100)
  7. If последний batch → finalize: read fresh videoCount from trendChannels docs → write embeddingStats + notifications + delete syncState
```

**Incremental sync (оптимизация ~150K+ Firestore reads/month):**
- Trends Sync записывает в embedding queue атомарно с video writes — 4 content fields (title, tags, description, thumbnail)
- Embedding sync читает только queue вместо full scan всех видео
- Queue canary: warning при >500 entries (persistent failures detection)

**Изоляция от sync pipeline:**
- Video sync pipeline знает про queue, но queue writes = best-effort (try/catch, не ломает sync)
- EmbeddingSync читает queue, обрабатывает, чистит — автономный pipeline
...
|---|---|---|---|
| Queue empty + embeddings exist | `info` | `scheduledEmbeddingSync:emptyQueue` | Все embeddings актуальны, нечего делать |
| Queue empty + no embeddings | `info` | `scheduledEmbeddingSync:fallbackFullScan` | Первый запуск — fallback на full scan |
...
| Queue >500 entries | `warn` | `readEmbeddingQueue:largeQueue` | Queue растёт — проверить persistent embedding failures |
| Pre-read docs failed | `warn` | `syncChannel:embeddingQueuePreReadFailed` | Best-effort degradation: queue writes продолжаются |
| Videos enqueued | `info` | `syncChannel:embeddingQueueEnqueued` | N видео с изменённым контентом добавлены в queue |
| Batch started | `info` | `embeddingSyncBatch:batchStart` | Начало обработки batch (для измерения duration) |
| Queue cleanup done | `info` | `embeddingSyncBatch:queueCleanup` | Успешные видео удалены из queue |
| Queue cleanup failed | `warn` | `embeddingSyncBatch:queueCleanupFailed` | Best-effort: видео останутся для retry |
| Channel totals read failed | `warn` | `embeddingSyncBatch:channelTotalsReadFailed` | Fallback на syncState totals |
| Sync finalized | `info` | `embeddingSyncBatch:finalized` | Итоги: generated, failed, cost |
...
| failCount update failed | `warn` | `processOneVideo:failCountUpdateFailed` | Firestore write error при записи failCount |
...
functions/src/embedding/
  ...
  embeddingSync.ts              ← discoverChannels() — collection group query → unique YouTube channels (used by fallback + backfill)
  embeddingQueue.ts             ← dirty queue utilities: isContentChanged (pure), enqueueVideoForEmbedding, readEmbeddingQueue
  scheduledEmbeddingSync.ts     ← thin launcher: read queue → write syncState → enqueue first batch (fallback to full scan on first run)
  embeddingSyncBatch.ts         ← self-chaining batch processor via Cloud Tasks (batch → counters → queue cleanup → chain/finalize)
  backfillEmbeddings.ts         ← Cloud Task chain for backfill (full scan via discoverChannels)
