...
**Стадия: Этапы 1–3 + searchDatabase реализованы.** Пять инструментов дают AI полный доступ к данным конкурентов:
...
- **searchDatabase** — free-text семантический поиск по всей базе. `generateQueryEmbedding` с `taskType: RETRIEVAL_QUERY` → cosine vector search → enrichment. [Feature doc](./5-search-database-tool.md)

Инфраструктура: глобальная коллекция `globalVideoEmbeddings` (4370 видео, 36 каналов), self-chaining batch pipeline через Cloud Tasks (`scheduledEmbeddingSync` → `embeddingSyncBatch`, daily 00:30 UTC), budget safeguard ($5/month), backfill pipeline. Shared `processOneVideo` (single thumbnail download). Coverage: 100% packaging, 98.1% visual (84 видео с битыми обложками).
