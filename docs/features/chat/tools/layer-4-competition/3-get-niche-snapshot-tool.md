...
- **Haiku не вызывает mentionVideo.** Тот же паттерн, что в searchDatabase: Haiku пишет `[title](mention://videoId)` с placeholder вместо реального ID. Known model limitation, не tool bug
