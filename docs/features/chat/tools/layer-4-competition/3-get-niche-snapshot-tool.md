...
| Файл | Назначение |
|------|-----------|
| `functions/src/services/tools/handlers/competition/getNicheSnapshot.ts` | Handler: date resolution, window, per-channel percentiles, aggregates |
...
| Файл | Кейсов |
|------|--------|
| `functions/src/services/tools/handlers/competition/__tests__/getNicheSnapshot.test.ts` | 14 (no date/videoId error, invalid date, date path, videoId+channelId, videoId-only scan, fallback resolveVideosByIds, videoId not found, custom windowDays, default windowDays, hidden video filtering, empty window, commonTags, dataFreshness, aggregates) |
| `functions/src/services/tools/handlers/detail/__tests__/getMultipleVideoDetails.titles.test.ts` | 7 (title resolution from videos/, notFoundTitles, merge titles+videoIds, deduplication, defensive string input, error on empty input, no YouTube API for title-resolved) |
