...
**Реализовано.** Главный рабочий инструмент конкурентной аналитики. AI запрашивает видео конкурентов из Trends data с фильтрами (каналы, даты, performanceTier) и сортировкой (дата, просмотры, дельты). Per-channel percentile тиры вычисляются в runtime. View deltas (24h/7d/30d) обогащаются из trend snapshots. Thumbnail descriptions (AI-generated visual summaries) обогащаются из globalVideoEmbeddings — surface-level визуальный контекст без дополнительных API-вызовов. Hidden videos фильтруются. Zero YouTube API cost.
...
5. LLM получает список видео с тирами, дельтами и `thumbnailDescription` (surface-level визуальный контекст) для анализа
6. Для глубокого визуального анализа обложек — `viewThumbnails(videoIds)` с ID из ответа
...
```typescript
{
    videos: [{
        ...
        tags: string[],
        thumbnailUrl: string,
        thumbnailDescription: string | null, // AI-generated visual summary (~200 words) from embeddings
...
Одно видео без thumbnailDescription ≈ 125 tokens. С thumbnailDescription ≈ 375 tokens (+250 за ~200 слов описания). Default 50 видео ≈ 18K tokens. Max 200 видео ≈ 75K tokens. Видео без embedding data получают `thumbnailDescription: null` (не увеличивают бюджет).
...
| Какой визуальный стиль? | `thumbnailDescription` — surface-level visual context из embeddings |
...
| Файл | Назначение |
|------|-----------|
| `functions/src/services/tools/handlers/competition/browseTrendVideos.ts` | Handler: per-channel percentiles, filters, delta + thumbnail description enrichment |
...
Firestore (global):
  globalVideoEmbeddings/{videoId}
    Fields: thumbnailDescription (AI-generated visual summary, ~200 words)
```
...
| Файл | Кейсов |
|------|--------|
| `functions/src/services/tools/handlers/competition/__tests__/browseTrendVideos.test.ts` | 16 (no channels error, default sort, channelIds filter, dateRange, performanceTier, invalid params, limit, hidden videos, totalMatched, dataFreshness, channels summary, thumbnailDescription enrichment + graceful failure, delta sort fallback) |
