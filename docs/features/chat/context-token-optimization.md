...
- [Agentic Architecture](./agentic-architecture.md) — agentic loop, tools
- [Multi-Provider Architecture](./multi-provider.md) — Gemini + Claude
...
- [ ] **Claude Auto Compaction** — серверная альтернатива L3 для Claude. Claude сам сжимает контекст внутри API-вызова (без отдельного Gemini Flash запроса). Подробности: [multi-provider.md, Stage 6](./multi-provider.md)
