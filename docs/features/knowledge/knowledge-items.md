...
- [x] **Этап 1: Data Layer** — типы, Firestore schema, CRUD сервисы, category registry
- [x] **Этап 2: Chat Tools** — `saveKnowledge`, `listKnowledge`, `getKnowledge` handlers + tool definitions
- [x] **Этап 3: Conclude Integration** — saveMemory, isConclude, strip content
- [x] **Этап 4: Context Integration** — флаги на документах видео/канала, инъекция в system prompt
- [x] **Этап 5: UI Foundation** — RichTextEditor (порт из MonkeyLearn), RichTextViewer, Zen Mode, KnowledgeCard, KnowledgeItemModal
- [x] **Этап 6: Video UI** — Watch Page: табы My Notes / AI Research, WatchPageKnowledge, edit modal
- [x] **Этап 7: Channel UI (Lab Page)** — sidebar route `/lab`, chip-row фильтры, sort, manual KI creation
- [ ] **Этап 8: Production Hardening** — тесты, review gates, edge cases ← YOU ARE HERE
...
Предыдущий подход (`concludeConversation.ts`) использовал отдельный Cloud Function, который заново читал историю из Firestore — cache miss для Anthropic моделей (10x дороже).

Текущий подход: кнопка "Memorize" отправляет обычный turn через тот же `aiChat` endpoint с `isConclude: true`. Кэш тёплый, модель та же, контекст уже загружен.
...
⚠️ **Firestore Composite Indexes (required for production):**
- `knowledgeItems`: `conversationId + category + videoId` (idempotency guard in saveKnowledge)
- `knowledgeItems`: `category + supersededBy + videoId` (auto-supersede query)
- `knowledgeItems`: `scope + createdAt` (listKnowledge by scope)
- `knowledgeItems`: `videoId + createdAt` (listKnowledge by video)
- `knowledgeItems`: `category + createdAt` (listKnowledge by category)
- `knowledgeItems`: `videoId + category + createdAt` (getKnowledge filter path)

Note: Firestore auto-creates single-field indexes but NOT composite ones. These must be deployed before production use. If a query fails with "requires an index" — Firestore logs include a direct link to create it.

#### MAX_AGENTIC_ITERATIONS

Текущий лимит: 10 итераций. Типичный Conclude: 3-5 KI + 1 Memory = 4-6 итераций. Укладывается с запасом.

#### Миграция с concludeConversation — ЗАВЕРШЕНА

`concludeConversation.ts` Cloud Function удалена. Вся логика Memorize реализована в tool handlers (`saveKnowledge` + `saveMemory`). Frontend вызывает `sendMessage()` с `isConclude: true` через тот же `aiChat` endpoint.
...
- **components/ui/organisms/RichTextEditor/** — Порт из MonkeyLearn: Tiptap editor (self-contained organism с extensions внутри)
...
- **services/tools/definitions.ts** — +3 tool definitions + conclude-only `saveMemory` tool
- **services/tools/handlers/knowledge/saveMemory.ts** — Handler: сохранение Memory с KI refs (conclude-only)
...
| 14 | concludeConversation.ts — УДАЛЕНА | Legacy CF удалена. Вся логика Memorize реализована в tool handlers (saveKnowledge + saveMemory) через `aiChat` endpoint |
