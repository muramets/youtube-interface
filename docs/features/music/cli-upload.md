...
Агент в CLI может автономно загружать треки в music library любого канала через новые tools `listMusicLibrary`, `addMusicGenre`, `addMusicTag`, `uploadTrack`. Поддерживаются vocal + instrumental варианты как единая пара. ID3-метаданные (title, artist, BPM, lyrics, embedded cover art) извлекаются автоматически — пользователя спрашивают только за тем, что отсутствует. Waveform peaks **не** генерируются при upload — frontend лениво декодирует аудио при первом рендере track card'а и сохраняет peaks обратно в Firestore, так что следующий рендер мгновенный.
...
- **Backend handlers:** `functions/src/services/tools/handlers/music/` (4 файла + общий `musicLibrary.ts` для validation + settings I/O)
...
- **Tests:** `functions/src/services/tools/handlers/music/__tests__/` — 28 tests (1106 total backend)
