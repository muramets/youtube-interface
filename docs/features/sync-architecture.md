...
| Каждый час (проверка) / 24ч (между попытками) | `useVideoFetchRetry` — ретраит failed custom видео (до 7 попыток) |
...
| Trends Sync — per-channel (Add Channel / sidebar «⋮ Sync» / header Sync на одном канале) | 1 unit (client-side channels.list для metadata) + ~5-10 units cloud (playlist + videos + subscriberCount + optional avatar) |
...
### Trends Sync — Frontend (все три пути dispatch'ат cloud function)
- `src/core/services/trendService.ts` — `addTrendChannel()` (metadata + setDoc), `syncChannelCloud()` (wrapper над `manualTrendSync` callable), `parseChannelInput()`
- `src/pages/Trends/Sidebar/AddChannelModal.tsx` — dispatch после создания канала
- `src/pages/Trends/Sidebar/hooks/useTrendsSidebar.ts` — `handleSyncChannel` (меню «⋮» на канале)
- `src/pages/Trends/hooks/useTrendsSync.ts` — кнопка Sync в header (все видимые каналы)
- `src/core/types/trends.ts` — `TrendVideo` interface (includes `likeCount`, `commentCount`)

### Trends Sync — Backend (единственное место, где крутится фактический YouTube-fetch для trend videos)
- `functions/src/trends/scheduledSync.ts` — daily cron (00:00 UTC)
- `functions/src/trends/manualSync.ts` — callable from frontend (используется Add Channel, per-channel refresh, header Sync)
...
### Video Fetch Retry
- `src/core/hooks/useVideoFetchRetry.ts` — retry logic for unavailable custom videos

**Что это:** когда пользователь привязывает YouTube ссылку к custom видео, а видео приватное, удалённое или ещё не опубликовано — `fetchStatus` становится `'failed'`. Video Fetch Retry терпеливо пытается получить данные снова.

**Механика:**
- Фильтр: только custom видео с `publishedVideoId`, где `fetchStatus !== 'success'` и `fetchRetryCount < 7`
- Попытка #0 (сразу): toast → *"Video not available yet. Will retry in 24 hours."*
- Попытки #1–6 (каждые 24ч): info-уведомление → *"Update #N for 'title'. Will automatically retry in 24 hours."*
- Попытка #7 (финальная): persistent error → *"Could not retrieve details."* + ссылка на страницу видео
- При успехе: YouTube данные → root поля видео, `fetchRetryCount` и `lastFetchAttempt` удаляются (`deleteField()`)
- Проверка запускается каждый час (`setInterval`), но между попытками для каждого видео — минимум 24ч

**Связь с Channel Sync:** Channel Sync пропускает custom видео с `fetchStatus: 'failed'`. За реабилитацию отвечает исключительно Video Fetch Retry. Когда retry получает `'success'` — видео снова попадает в Channel Sync.
