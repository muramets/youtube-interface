...
1. **Добавление каналов** — пользователь вставляет ссылку на YouTube-канал (URL, @handle, или channel ID). Система резолвит ID и загружает метаданные (1 quota unit), канал моментально появляется в sidebar. Дальше в фоне Cloud Function `manualTrendSync` подтягивает все видео + создаёт первый snapshot — ровно тот же код, что крутится ежедневным cron'ом и по кнопке Sync в header.
2. **Автоматическая синхронизация** — каждый день в 00:00 UTC Cloud Scheduler запускает синк всех каналов. Те же каналы можно синкнуть вручную (header кнопка — все видимые, меню «⋮» в sidebar — один канал). Любой ручной dispatch идёт через тот же `manualTrendSync`.
