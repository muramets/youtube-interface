...
Адаптивная система для производительности с большими датасетами (1000+ видео):

| Масштаб | Поведение |
|---------|-----------|
| `< 5%` | Всегда dots (canvas) |
| `5% – 25%` | **Dynamic LOD**: thumbnails, если видимых видео ≤ 80; иначе dots |
| `≥ 25%` | Всегда thumbnails + view count labels |

Виртуализация: viewport culling по обеим осям (X + Y) с буфером 500px. Видео за пределами экрана не рендерятся в DOM.
